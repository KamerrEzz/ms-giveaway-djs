...
import { APIActionRowComponent, APIButtonComponent, APIButtonComponentWithCustomId, APIEmbed, APIMessageActionRowComponent, APIMessageComponentEmoji, APIPartialEmoji, ButtonStyle } from "discord-api-types/v10";


export default new class Message {

    create(channel_id: string, body: RESTPostAPIChannelMessageJSONBody){
...
export function embed(title?: string, description?: string, color?: `#${string}` | number): APIEmbed {
    return {
        title,
        description,
...
function parseColor(color: `#${string}` | number | undefined): number | undefined {
